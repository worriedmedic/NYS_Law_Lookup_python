...
class App(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        ttk.Label(self, text="Statutes").grid(row=4, column=0, sticky="w", pady=(10, 0))
        self.statute_list = tk.Listbox(self, height=8)
        self.statute_list.grid(row=4, column=0, columnspan=3, sticky="nsew")
        statute_scroll = ttk.Scrollbar(self, command=self.statute_list.yview)
        statute_scroll.grid(row=4, column=3, sticky="ns")
        ...
        self.output.grid(row=5, column=0, columnspan=3, sticky="nsew", pady=(10, 0))
        scrollbar = ttk.Scrollbar(self, command=self.output.yview)
        scrollbar.grid(row=5, column=3, sticky="ns", pady=(10, 0))
    ...
    def _run_worker(self, func):
        def task():
            ...
            try:
                ...
                self.master.after(0, lambda: messagebox.showerror("Error", str(exc)))
    ...
    def _run_list_statutes(self, show_output=False):
        ...
        def work():
            ...
            sections = extract_sections_from_lines(output)
            if not sections:
